...
class S_Interp(nn.Module):
    def __init__(self, orig_d_ts, tt_max, embed_dim):
        super(S_Interp, self).__init__()

        self.tt_max = tt_max
        self.ref_t = torch.linspace(0, 1., self.tt_max)
    ...
    def forward(self, x_ts, x_ts_mask, ts_tt_list, rec_mask, reconstruction=False):
        ...
        a = torch.ones([self.d_dim, tt_len, output_dim]).type_as(x_ts)
...
class Cross_Interp(nn.Module):
    def __init__(self, orig_d_ts):
        super(Cross_Interp, self).__init__()
        self.d_dim = orig_d_ts
        self.activation = nn.Sigmoid()
        self.cross_channel_interp = torch.empty(
            self.d_dim, self.d_dim)
