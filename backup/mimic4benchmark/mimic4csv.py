...
def read_icd_diagnoses_table(mimic4_path):
    # mimic-iv use icd_code and icd_version instead of icd9_code
    codes = dataframe_from_csv(os.path.join(
        mimic4_path, 'd_icd_diagnoses.csv'))

    # only keep ICD-9 codes
    codes = codes[codes['icd_version'] == 9][['icd_code', 'long_title']]
...
def count_icd_codes(diagnoses, output_path=None):
    codes = diagnoses[['icd_code', 'icd_version', 'long_title']
                      ].drop_duplicates().set_index(['icd_code', 'icd_version'])
    codes['count'] = diagnoses.groupby(['icd_code', 'icd_version'])[
        'stay_id'].count()
...
def filter_admissions_on_nb_icustays(stays, min_nb_stays=1, max_nb_stays=1):
    ...
    stays = stays.merge(to_keep, how='inner',
...
def read_events_table_and_break_up_by_subject(mimic4_path, table, output_path,
    ...
    def write_current_observations():
        ...
        if not os.path.exists(fn) or not os.path.isfile(fn):
            ...
            f.close()
        w = csv.DictWriter(open(fn, 'a'), fieldnames=obs_header,
