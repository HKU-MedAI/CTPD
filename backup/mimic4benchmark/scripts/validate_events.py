...
def main():
    ...
    no_icustay = 0
    # empty ICUSTAY_IDs are recovered according to stays.csv files (given hadm_id)
    recovered = 0
    # empty ICUSTAY_IDs that are not recovered. This should be zero.
    could_not_recover = 0
    ...
    parser.add_argument('--subjects_root_path', type=str, required=False,
                        default='data/root', help='Directory containing subject subdirectories.')
    ...
    for subject in tqdm(subjects, desc='Iterating over subjects'):
        ...
        # mimic-iv use lower case column names
