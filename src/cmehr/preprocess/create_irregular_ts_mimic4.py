...
from cmehr.ext.mimic4benchmark.readers import InHospitalMortalityReader, PhenotypingReader, Reader
...
def create_irregular_ts():
    ...
    if args.task == 'ihm':
        train_reader = InHospitalMortalityReader(
            dataset_dir=MIMIC4_IHM_PATH / "train",
            listfile=MIMIC4_IHM_PATH / "train_listfile.csv",
        ...
        val_reader = InHospitalMortalityReader(
            dataset_dir=MIMIC4_IHM_PATH / "train",
            listfile=MIMIC4_IHM_PATH / "val_listfile.csv",
            period_length=args.period_length,
            columns=variables
        )

        test_reader = InHospitalMortalityReader(
            dataset_dir=MIMIC4_IHM_PATH / "test",
            listfile=MIMIC4_IHM_PATH / "test_listfile.csv",
            period_length=args.period_length,
            columns=variables
        )
    elif args.task == "pheno":
        train_reader = PhenotypingReader(
            dataset_dir=MIMIC4_PHENO_24H_PATH / "train",
            listfile=MIMIC4_PHENO_24H_PATH / "train_listfile.csv",
            columns=variables
        )
        val_reader = PhenotypingReader(
            dataset_dir=MIMIC4_PHENO_24H_PATH / "train",
            listfile=MIMIC4_PHENO_24H_PATH / "val_listfile.csv",
            columns=variables
        )
        test_reader = PhenotypingReader(
            dataset_dir=MIMIC4_PHENO_24H_PATH / "test",
            listfile=MIMIC4_PHENO_24H_PATH / "test_listfile.csv",
            columns=variables
        )
    else:
        raise ValueError("Task is invalid")
    ...
    if args.task == 'ihm':
        ...
        normalizer_state = str(
            ROOT_PATH /
            f"cmehr/ext/mimic4models/phenotyping/ph_ts{args.timestep}.input_str:{args.imputation}.start_time:zero.normalizer"
        )
    ...
    if "CXR" in args.modality_type:
        ...
        if args.task == 'ihm':
            ...
            mimic4_cxr_csv = MIMIC4_CXR_CSV_PHENO
            list_csv_dir = MIMIC4_PHENO_24H_PATH
        else:
            raise ValueError("Task is invalid")
