    ...
    for patient in tqdm(patients, desc='Iterating over patients in {}'.format(partition)):
        ...
        for ts_filename in patient_ts_files:
            with open(os.path.join(patient_folder, ts_filename)) as tsfile:
                ...
                readmission_30d = int(stay_df[stay_df.ICUSTAY_ID == stay_id].iloc[0].readmission_30d)
                # check if the patient died in the ICU
                ...
                # TODO: Here we use sensitive attributes from the label df, while we could directly use the raw values in stays.csv
                insurance = int(label_df.iloc[0]['Insurance'])
                # in mimiciii, others is a category in race
                if insurance <= 0:
                    continue
