...
def process_partition(args, oud_codes,
            ...
            with open(os.path.join(patient_folder, ts_filename)) as tsfile:
                ...
                for index, row in diagnoses_df.iterrows():
                    ...
                    if code in oud_codes:
                        oud_label = 1
                        pos += 1
                xty_triples.append((output_ts_filename, n_hours, oud_label, insurance, race, gender, age, marital_status))
...
def main():
    ...
    parser.add_argument('--output_path', type=str,
                        default=os.path.join(os.path.dirname(__file__), '../../data/oud/'),
    ...
    # we use SUD to replace OUD, because SUD includes more codes than OUD
    oud_group = 'Substance-related disorders'
    oud_codes = definitions[oud_group]['codes']

    if not os.path.exists(args.output_path):
        os.makedirs(args.output_path)

    process_partition(args, oud_codes, "test")
    process_partition(args, oud_codes, "train")
