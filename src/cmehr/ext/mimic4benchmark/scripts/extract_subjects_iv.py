...
from mimic4benchmark.mimic3csv import *
...
patients = read_patients_table(f'{args.mimic3_path}/patients.csv')

admits = read_admissions_table(f'{args.mimic3_path}/admissions.csv')


stays = read_icustays_table(f'{args.mimic3_path}/icustays.csv')
if args.verbose:
...
diagnoses = read_icd_diagnoses_table(f'{args.mimic3_path}')

diagnoses = filter_diagnoses_on_stays(diagnoses, stays)
diagnoses.to_csv(os.path.join(args.output_path,
                 'all_diagnoses.csv'), index=False)

count_icd_codes(diagnoses, output_path=os.path.join(
    args.output_path, 'diagnosis_counts.csv'))

phenotypes = add_hcup_ccs_2015_groups(diagnoses, yaml.load(
    open(f'{args.phenotype_definitions}', 'r'), Loader=yaml.FullLoader))
...
if args.test:
    ...
    print('Using only', stays.shape[0],
          'stays and only', args.event_tables[0], 'table')
