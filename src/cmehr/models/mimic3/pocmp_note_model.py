...
class POCMPNoteModule(MIMIC3LightningModule):
    '''
    ...
    def forward(self, x_ts, x_ts_mask, ts_tt_list,
        ...
        if self.use_prototype:
            ...
            concat_feat = torch.cat([concat_text_slot, concat_text_feat], dim=1)
            ...
            concat_feat = concat_text_feat

        fusion_feat = self.fusion_layer(concat_feat)

        # STEP 7: make prediction
        if self.use_prototype:
            num_text_tokens = concat_text_feat.size(1) + concat_text_slot.size(1)
            ...
            num_text_tokens = concat_text_feat.size(1)
        
        # num_text_tokens = concat_text_slot.size(1) + concat_text_feat.size(1)
        # last_ts_feat = torch.mean(fusion_feat[:, :num_text_tokens, :], dim=1)
        # last_text_feat = torch.mean(fusion_feat[:, num_text_tokens:, :], dim=1)
        ...
        text_pred_tokens = fusion_feat[:, num_text_tokens:, :]
        ...
        output = self.out_layer(last_text_feat)
        ...
        if self.task in ['ihm', 'readm']:
            if labels != None:
                ...
                loss_dict["total_loss"] = ce_loss + cont_loss * self.lamb1 + text_recon_loss * self.lamb3
                return loss_dict
